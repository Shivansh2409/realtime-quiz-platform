...
const setupSocketHandlers = (io) => {
  io.on('connection', (socket) => {
    ...
    socket.on('admin-join', async ({ sessionId }) => {
      try {
        ...
        if (session) {
          ...
          socket.isAdmin = true;
          console.log(`Admin joined session: ${session.code}`);
        }
      } catch (error) {
    ...
    socket.on('student-join', async ({ sessionCode, studentName, visitorId }) => {
      try {
        const session = await Session.findOne({ code: sessionCode.toUpperCase() }).populate('quiz');
        ...
        let participant = session.participants.find(p => p.visitorId === visitorId);
        ...
        if (session.status === 'active' && session.currentQuestionIndex >= 0) {
          ...
          const elapsed = Date.now() - new Date(session.questionStartTime).getTime();
          const remainingTime = Math.max(0, question.timerDuration * 1000 - elapsed);
    ...
    socket.on('submit-answer', async ({ sessionCode, visitorId, questionIndex, answer, timeSpent }) => {
      try {
        const session = await Session.findOne({ code: sessionCode }).populate('quiz');
        ...
        const participant = session.participants.find(p => p.visitorId === visitorId);
        if (!participant) {
          return socket.emit('error', { message: 'Participant not found' });
        }

        // Check if already answered
        const existingResponse = participant.responses.find(r => r.questionIndex === questionIndex);
        if (existingResponse) {
        ...
        console.log(`Answer submitted by ${participant.name} for question ${questionIndex}`);
      } catch (error) {
    ...
    socket.on('disconnect', async () => {
      ...
      if (socket.sessionCode && socket.visitorId && !socket.isAdmin) {
        try {
          ...
          if (session) {
            const participant = session.participants.find(p => p.visitorId === socket.visitorId);
            if (participant) {
...
const calculateLeaderboard = (session) => {
  return session.participants.map(participant => {
    ...
    const totalTime = participant.responses.reduce((sum, r) => sum + (r.timeSpent || 0), 0);
